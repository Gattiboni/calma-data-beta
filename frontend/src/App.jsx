...
// Formatador curto para RECEITA (somente): R$9,5K para milhares; R$500 para centenas (sem separador decimal)
...
function Icon({name, className='w-4 h-4 text-elegant'}){
  ...
  if(name==='home') return (
    <svg className={className} viewBox="0 0 32 32"><circle cx="16" cy="16" r="13" {...S}/><path {...S} d="M8 16l8-7 8 7M11 18v7h10v-7"/></svg>
  )
  if(name==='info') return (
    <svg className={className} viewBox="0 0 32 32"><circle cx="16" cy="16" r="13" {...S}/><path {...S} d="M16 12h.01M15 16h2v6h-2"/></svg>
  )
  if(name==='settings') return (
    <svg className={className} viewBox="0 0 32 32"><circle cx="16" cy="16" r="13" {...S}/><path {...S} d="M8 11h14M8 16h10M8 21h12"/></svg>
  )
...
function KPI({title, value, bg=KPIGREEN, icon}){
  return (
    <div className="kpi kpi-colored" style={{backgroundColor:bg}}>
      <div className="kpi-title flex items-center gap-2"><Icon name={icon} className="w-3.5 h-3.5 text-white/90"/> {title}</div>
      <div className="kpi-value mt-1">{value}</div>
    </div>
  )
}

// Dados demo (inalterado omitido por brevidade)
function buildDemo(range){
  ...
  return { kpis:{ receita:71400, reservas:32, diarias:51, clicks:5290, impressoes:148000, cpc:0.12, custo:615 }, acq:{ metric:'users', points: acqPoints }, revUH:{ points: revPoints } }
}
...
function EmptyState({title='Sem dados para o período', subtitle='Ajuste o intervalo ou verifique integrações.'}) {
  return (
    <div className="h-full w-full flex items-center justify-center text-elegant/80 text-sm">
      <div>
...
function parseDDMMYY(s){ const [dd,mm,yy] = s.split('/'); return new Date(2000+parseInt(yy,10), parseInt(mm,10)-1, parseInt(dd,10)) }
...
function colorForKey(key, allKeys){ const idx = allKeys.indexOf(key); return PALETTE[(idx>=0? idx: 0) % PALETTE.length] }
function abbrev(name){ if(!name) return ''; if(name.toLowerCase()==='extras') return 'Extras'; return name.split(/\s+/).map(w=> w[0]?.toUpperCase()).join('').slice(0,4) }

function RevUHSummary({curr, prev}){
  // usa dados brutos (não bucketizados)
  const rows = useMemo(()=>{
    const currTotals = sumBySeries(curr?.points)
    const prevTotals = sumBySeries(prev?.points)
    return makeDeltaRows(currTotals, prevTotals)
  },[curr, prev])
  const keys = useMemo(()=> getSeriesKeys((curr?.points||[]).map(p=> ({date:p.date, ...p.values}))), [curr])
  return (
    <div className="card">
      ...
      <div className="card-body" style={{height:320, overflowY:'auto'}}>
        <table className="table-mini text-sm w-full">
          ...
          {rows.map(r=> (
            <tr key={r.key}>
              <td className="flex items-center gap-2"><span className="inline-block w-3 h-3 rounded-sm" style={{backgroundColor:colorForKey(r.key, keys)}}></span>{abbrev(r.key)}</td>
...
function RevUHInsights({curr, prev}){
  const content = useMemo(()=>{
    const currTotals = sumBySeries(curr?.points)
    const prevTotals = sumBySeries(prev?.points)
    const rows = makeDeltaRows(currTotals, prevTotals)
    ...
    return [
      ...
      gains[0] ? <p key="p2">O maior crescimento ficou com <strong>{gains[0].key}</strong>, variação de <strong>+{gains[0].delta.toFixed(1)}%</strong> em relação ao período anterior.</p> : null,
      drops[0] ? <p key="p3">A maior retração ocorreu em <strong>{drops[0].key}</strong>, com <strong>{drops[0].delta.toFixed(1)}%</strong>.</p> : null,
      <p key="p4">A tendência geral frente ao período anterior é de <strong>{d>=0?'+':''}{d.toFixed(1)}%</strong>.</p>
    ].filter(Boolean)
  },[curr, prev])
  return (
    <div className="card">
...
function AcquisitionSummary({curr, prev}){
  const rows = useMemo(()=>{
    const currTotals = sumBySeries(curr?.points)
    const prevTotals = sumBySeries(prev?.points)
    return makeDeltaRows(currTotals, prevTotals)
  },[curr, prev])
  const keys = useMemo(()=> getSeriesKeys(translatePoints(curr?.points).map(p=> ({date:p.date, ...p.values}))), [curr])
  return (
    <div className="card">
      <div className="card-header">Quadro-Resumo</div>
      <div className="card-body" style={{height:320, overflowY:'auto'}}>
        <table className="table-mini text-sm w-full">
          ...
          {rows.map(r=> {
            ...
            return (
              <tr key={r.key}>
                <td className="flex items-center gap-2"><span className="inline-block w-3 h-3 rounded-sm" style={{backgroundColor:colorForKey(name, keys)}}></span>{abbrev(name)}</td>
...
function AcquisitionInsights({curr, prev}){
  const content = useMemo(()=>{
    const currTotals = sumBySeries(curr?.points)
    const prevTotals = sumBySeries(prev?.points)
    const rows = makeDeltaRows(currTotals, prevTotals)
...
function AcquisitionLine({series, spanDays}){
  // traduz e agrega conforme período
  ...
  const tickFmt = (d)=> spanDays>90? d : d // já formatado em bucketize
  return (
    <div className="card">
        ...
        {!hasData ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData}>
              ...
              <XAxis dataKey="date" hide={false} tick={{fontSize:12}} tickFormatter={tickFmt}/>
...
function RevenueByUH({data, spanDays}){
  const chartData = useMemo(()=> data?.points? data.points.map(p=> ({ date: p.date, ...p.values })) : [],[data])
  const keys = useMemo(()=> getSeriesKeys(chartData), [chartData])
  const hasData = chartData.length>0 && keys.length>0
  const tickFmt = (d)=> spanDays>90? monthAbbrevFromDDMMYY(d) : d
  return (
    <div className="card">
      ...
      <div className="card-body" style={{height:320}}>
        {!hasData ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData}>
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis dataKey="date" tick={{fontSize:12}} tickFormatter={tickFmt}/>
        ...
        {!hasData ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData}>
              ...
              <Legend/>
              {keys.map((k,i)=> (
                <Bar key={k} dataKey={k} stackId="a" fill={PALETTE[i%4]} />
              ))}
            </BarChart>
          </ResponsiveContainer>
        )}
        ...
        <table className="min-w-full text-sm" style={{fontFamily:'Inter, system-ui, sans-serif'}}>
          <thead>
            <tr className="text-left">
              {['Campanha','Cliques','Impressões','CTR','CPC','Custo','Receita','ROAS'].map(h=> <th key={h} className="py-2 pr-4">{h}</th>)}
            </tr>
          </thead>
          <tbody>
            {rows?.map((r,i)=> (
...
export default function App(){
  ...
  const spanDays = useMemo(()=> { const s = new Date(range.start); const e = new Date(range.end); return Math.round((e-s)/(1000*60*60*24))+1 },[range.start, range.end])
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <main className="flex-1 overflow-auto p-6 space-y-6">
          ...
          <section className="kpi-grid">
            <KPI title="RECEITA" value={loading? '—' : formatBRLShort(data?.kpis?.receita)} icon="money"/>
            <KPI title="RESERVAS" value={loading? '—' : data?.kpis?.reservas} icon="bag"/>
            <KPI title="DIÁRIAS" value={loading? '—' : data?.kpis?.diarias} icon="moon"/>
            <KPI title="CLICKS" value={loading? '—' : data?.kpis?.clicks?.toLocaleString('pt-BR')} icon="click"/>
            <KPI title="IMPRESSÕES" value={loading? '—' : data?.kpis?.impressoes?.toLocaleString('pt-BR')} icon="eye"/>
            <KPI title="CPC" value={loading? '—' : `R$${(data?.kpis?.cpc||0).toFixed(2).replace('.',',')}`} icon="coin"/>
            <KPI title="CUSTO" value={loading? '—' : `R$${(data?.kpis?.custo||0).toFixed(2).replace('.',',')}`} icon="cost"/>
          </section>

          {/* Linha Aquisição + painéis */}
          <section className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            <div className="lg:col-span-6"><AcquisitionLine series={data?.acq} spanDays={spanDays}/></div>
            <div className="lg:col-span-3"><AcquisitionSummary curr={data?.acq} prev={data?.prevAcq}/></div>
            <div className="lg:col-span-3"><AcquisitionInsights curr={data?.acq} prev={data?.prevAcq}/></div>
          </section>

          {/* Linha Receita por UH + painéis */}
          <section className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            <div className="lg:col-span-6"><RevenueByUH data={data?.revUH} spanDays={spanDays}/></div>
            <div className="lg:col-span-3"><RevUHSummary curr={data?.revUH} prev={data?.prevRevUH}/></div>
            <div className="lg:col-span-3"><RevUHInsights curr={data?.revUH} prev={data?.prevRevUH}/></div>
          </section>
